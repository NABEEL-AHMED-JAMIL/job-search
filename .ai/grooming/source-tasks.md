...
| `task_form_field` | `task_form_field_id` PK (**shares** `task_form_source_seq`), `task_form_id` FK `ON DELETE CASCADE`, `tag_key`, `tag_parent`, `label`, `field_type`, `required`, `default_value`, `help_text`, `field_options`, `position` | `V19__task_form_builder.sql:27-42`. The only `ON DELETE` clause in the schema. `field_options` is one choice per line; on each line the FIRST `=` optionally separates the stored VALUE from the displayed LABEL (`lines=JSON Lines (one object per line)`), and a line with no `=` is both halves — which is what keeps every form written before V36 reading exactly as it did. The label is the unsplit remainder, so it may contain `=`, `:` or `,` freely; a `=` inside a VALUE is the one thing the format cannot express and the authoring dialog refuses it by name. See `V36__select_choice_key_label.sql`. |
