...
| 6 | `source-tasks` | `taskList`, `addTask`, `editTask/:taskDetailId` | `tasks`, `tasks/new`, `tasks/:taskDetailId/edit`, `settings/forms` | `sourceTask.json/listSourceTask`, `/addSourceTask`, `/updateSourceTask`, `/deleteSourceTask`, `/fetchSourceTaskWithSourceTaskId`, `/fetchAllLinkJobsWithSourceTaskId`; `setting.json/xmlCreateChecker`, `/appSetting`, `/fetchSubLookupByParentId`; `pipeline.json/list`, `/listPipelines`, `/listForTopic`, `/definition`, `/save`, `/delete` (the `taskForm.json/*` spellings still answer) | SourceTask, SourceTaskPayload, SourceTaskType, Pipeline, PipelineField, LookupData | partial |
...
| 20 | `page-access-profiles` | -- | `admin/access-profiles` (+ `unauthorized` "Request access", `data.pageKey` + `pageGuard` on 20 shell routes, the Access column / filter / row action on `admin/users`) | `pageAccess.json/catalogue`, `/mine`, `/profiles`, `/addProfile`, `/updateProfile`, `/deleteProfile`, `/setDefault`, `/people`, `/assignProfile`, `/setPageAccess`, `/clearPageAccess`, `/requestAccess`; enforced for every gated API group by `security/PageAccessInterceptor` | PageAccessProfile, UserPageAccess, AppUser (`page_access_profile_id`) | new |

20 features. Counts by status: **8 migrated, 6 partial, 5 new, 1 not-migrated.** *(Was 18 features
and 3 `new` until row 19 `analytics-studio` was added on 2026-09-08; row 20 `page-access-profiles`
was added on 2026-09-17.)*

> **Row 20 `page-access-profiles` added 2026-09-17.** `new` for the same narrow reason as row 19:
> the old app gates by role only and has no notion of a page a tenant user may not open. Unlike
> row 19 this one is complete rather than a first slice -- catalogue, schema (V40, V41), server
> enforcement, console guard and menu, management screen with a People × pages grid, per-person
> exceptions, notifications, e2e. Groomed and synthesised after the fact:
> [../grooming/page-access-profiles.md](../grooming/page-access-profiles.md),
> [../synthesis/page-access-profiles.md](../synthesis/page-access-profiles.md).
...
| **Task Forms (pipeline form definitions)** | `features/settings/forms/` at `settings/forms`, consumed by the task editor to author XML tag rows through `syncFormToTags`. **Renamed Pipelines and moved under a topic on 2026-09-18** -- see the row below | `taskForm` appears nowhere in `scheduler1/src` |
...
| **Pipelines belong to a topic; a task picks the topic, then the pipeline** (added 2026-09-18) | `process`: V43 renames `task_form`→`pipeline` (surrogate key `pipeline_key`; `pipeline_id` stays the public id the worker routes on), `task_form_field`→`pipeline_field`, and adds `pipeline.source_task_type_id`; `Pipeline`/`PipelineField`/`PipelineRepository`/`PipelineServiceImpl`/`PipelineRestApi` at `/pipeline.json/*`; the topic is required and must be the workspace's own; a topic with pipelines refuses deletion. `scheduler1/next`: `features/settings/pipelines/` at `settings/pipelines` (Topic column, filter, "No topic" tile, topic picker in the dialog); the Kafka pane's Topics table counts and links each topic's pipelines; the task editor offers pipelines only after a topic is picked and clears one that is not on it; Source Tasks filters by topic then pipeline; menu entry **Pipelines** | Before V43 a task carried two independent pointers -- `source_task.source_task_type_id` and a free-string `pipeline_id` -- and nothing tied a pipeline to a topic |
| **Access profiles -- which pages a tenant user may open** (added 2026-09-17) | Row 20. `process/security/PageAccessInterceptor.java` answers 403 for a withheld page's API group; `model/enums/PageKey.java` is the catalogue of ten gated pages with their API prefixes. `scheduler1/next`: `core/auth/auth.guard.ts:53` (`pageGuard`), `features/admin/access-profiles/` (cards + People × pages grid with amber-marked per-person exceptions), Access column and filter on `features/admin/users/` | No `pageAccess`, `PageKey` or per-page gating anywhere in `scheduler1/src`; the old app's only lever is the role |
| **Worker callbacks proved per run** (added 2026-09-17) | `process/security/RunCallbackTokens.java`: each dispatch mints `cbt_<attempt>.<runId>.<random>`, keeps only its SHA-256 on the `job_queue` row (schema `V42.0-run-callback-token`), and sends the token in the Kafka message as `callbackToken`. `api/NotifyResetApi.java:54` verifies it -- wrong job, finished run, expired, mismatch all answer the same 401 -- and retires it on an accepted terminal change. `WORKER_CALLBACK_TOKEN` is now a legacy fallback for hash-less rows only; the app no longer refuses to start without it. Groomed at [../grooming/worker-callback-tokens.md](../grooming/worker-callback-tokens.md) | The old stack authenticated every worker with one shared secret (`job-search/docker-compose.yml`), and `job_queue` had no token columns before V42 |
| **Platform storage on S3 under one AWS identity; SES is the only mail path** (added 2026-09-16) | `process`: two platform buckets, `etl-avatar` (avatars) and `etl-config` (Kafka key material), on S3 -- LocalStack locally -- seeded by `StorageConnectionBootstrap` from the single `AWS_ENDPOINT/REGION/ACCESS_KEY/SECRET_KEY` set that SES also signs with. `MINIO_*`, `AWS_S3_*`, `AWS_SES_*`, `MAIL_HOST..PASSWORD`, `spring.mail.*`, `SmtpMailSender` and `AzureBlobConfig` are deleted; MinIO and Azure remain **per-connection** providers a tenant adds on `admin/storage`. `etl-bucket` is gone | The old app had a global MinIO account (`MINIO_*`) and an SMTP relay; neither exists now |
| **Reports: one column per outcome, skipped and missed included** (added 2026-09-17) | `process/model/service/impl/QueryService.runReportRows` admits a run by `start_time` **or** `skip_time`, so Skip and Missed rows reach the page. `scheduler1/next/features/reports/`: Task health has Completed / Failed / Interrupted / Skipped / Missed columns (`OUTCOME_COLUMNS` in `reports.ts`), the Runs tile says how many never started, and the outcome filter, donut and day chart carry all five | Before 2026-09-17 the query filtered `WHERE start_time IS NOT NULL`, so a skipped run was six fewer runs rather than six skipped ones |
| **Tenants screen: copyable code, admin contact** (added 2026-09-17) | `features/admin/tenants/`: the code sits under the name as a click-to-copy chip (`.tenant-code`); an Admin column and card line name the workspace's first active tenant admin (`TenantDto.adminName/adminEmail`); row menu gains "Access profiles" (deep link to the grid scoped to that tenant) and "Copy code"; counts centred. `features/admin/users/`: copy buttons on email and phone in the table, not only on the cards | Old `tenants` screen has no copy affordance and no admin contact |
| **Users are told when they are created; admins too** (added 2026-09-16) | `AppUserServiceImpl.addUser` notifies the new user (welcome, with the one-time password rule), the acting admin (record) and the workspace's peer admins. The dead Status picker was dropped from the user dialog (`14e8a41`) | No creation notification of any kind in `scheduler1/src` |
